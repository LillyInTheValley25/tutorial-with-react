...
directory. Then run the following command to install the dependencies and run the app:
...
This will start the Vite server, click the link which is output to the console to see your
app in the browser.

:::warning
VSCode users
If you have the Deno extension for VSCode installed, you may notice that the editor highlights some errors in the code. This is because the app created by Vite is designed with Node in mind and so uses conventions that Deno does not (such as 'sloppy imports' - importing modules without the file extension).

Disable the Deno extension for this project to avoid these errors
:::
...
If you run `deno task dev` now and visit [127.0.0.1:8000/api/dinosaurs](http://127.0.0.1:8000/api/dinosaurs), in your browser you should see a JSON response of all of the dinosaurs.
